...
async function simulateMerge() {
  ...
  // Helper to generate occurrence-based keys for de-duplication
  ...
  console.log('Loading Otel Yedek...');
  ...
  ertRows.forEach(r => {
    ...
    if (!masterMap.has(k)) {
      ...
      p2Count++;
      if (k.endsWith('___1') || k.endsWith('___2') || k.endsWith('___3')) {
        p2DuplicateKept++;
      }
    }
  });
  ...
  console.log(`\n=== SIMULATION RESULTS ===`);
  console.log(`Otel Yedek rows added (P1): ${p1Count}`);
  console.log(`Ertaşlar rows added (P2): ${p2Count} (including ${p2DuplicateKept} duplicate occurrences that were previously discarded)`);
  console.log(`Mallar rows added (P3): ${p3Count}`);
  console.log(`Total Master Map Size: ${masterMap.size}`);

  const allTxs = Array.from(masterMap.values());
  const ertTxs = allTxs.filter(t => t.supplier === 'ERTAŞLAR');
  console.log(`Total ERTAŞLAR transactions in simulation: ${ertTxs.length}`);
}

simulateMerge().catch(console.error);
