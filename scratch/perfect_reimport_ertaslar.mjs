...
// Parse number accurately without stripping decimals!
  ...
  for (const line of lines) {
    ...
    if (parts.length >= 3) {
      ...
      if (!isNaN(val) && val > 0) {
        ...
        priceListItems.push({ date: isoDate, product: prodName, unit: unit, price: val.toFixed(2) });
      }
    }
  }
...
async function fetchTutedWebExcel(url, isoDate) {
  ...
  for (let i = 2; i < rows.length; i++) {
    ...
    if (!isNaN(price) && price > 0) {
      priceMap[name] = price;
      priceListItems.push({ date: isoDate, product: name, unit: unit, price: price.toFixed(2) });
    }
  }
  return { priceMap, priceListItems };
}
...
async function main() {
  console.log('1. Reading Excel with RAW: TRUE to fix all decimal values...');
  const excelPath = "C:\\Users\\Baran\\Downloads\\Ertaşlar - Ram - Şimal 04.08.2026 (1).xlsx";
  const excelBuf = fs.readFileSync(excelPath);
  const wbExcel = XLSX.read(excelBuf, { type: 'buffer', raw: true });
  const sheet = wbExcel.Sheets['Sheet'];
  const excelRows = XLSX.utils.sheet_to_json(sheet, { header: 1, raw: true });
  const dataRows = excelRows.slice(1).filter(r => r[0] && r[1] && r[4] !== undefined);

  console.log(`Loaded ${dataRows.length} valid rows from Excel.`);

  console.log('\n2. Parsing PDF price lists for April, May, June...');
  const allPriceMaps = {};
  const newPriceLists = {};
  ...
  for (const [isoDate, pdfName] of Object.entries(dateToPdfFile)) {
    ...
    newPriceLists[isoDate] = priceListItems;
  }

  console.log('3. Fetching Web TÜTED for July dates...');
  const webIndex = await fetchTutedWebIndex();

  const uniqueExcelDates = [...new Set(dataRows.map(r => excelDateToIso(r[0])))].sort();

  for (const isoDate of uniqueExcelDates) {
  ...
  console.log('\n4. Re-building all ERTAŞLAR transactions with exact numbers...');
  ...
  dataRows.forEach((r, idx) => {
    ...
    const rawDepo = String(r[8]).trim();
    const hotel = depoToHotel[rawDepo] || rawDepo;
    ...
    const priceMap = allPriceMaps[isoDate];
    let tutedPrice = null;
    if (priceMap) {
      tutedPrice = findMatchingPrice(product, priceMap);
    }

    let supplyPrice = 0;
    if (tutedPrice !== null && tutedPrice > 0) {
      supplyPrice = Math.round(tutedPrice * marginRate * 100) / 100;
    } else {
      // If no TÜTED price exists, supplyPrice = buyPrice so Fark = 0 TL!
      supplyPrice = buyPrice;
    }

    newTransactions.push({
      id: 1785700000000 + idx + 1,
      date: isoDate,
      supplier: 'ERTAŞLAR',
      hotel: hotel,
      product: product,
  ...
  console.log(`Generated ${newTransactions.length} clean ERTAŞLAR transactions.`);

  // Verify Row 1790 (Row index 1788 in 0-indexed dataRows)
  const row1790Tx = newTransactions[newTransactions.length - 1];
  console.log('\n=== VERIFICATION OF ROW 1790 (MARUL LOLO ROSSO KIRMIZI 27.07.2026) ===');
  console.log(JSON.stringify(row1790Tx));

  console.log('\n5. Saving clean transactions and priceLists to Firebase...');
  const docRef = doc(db, 'storage', 'appData');
  const docSnap = await getDoc(docRef);
  if (!docSnap.exists()) process.exit(1);
  const fbData = docSnap.data();

  // Keep non-ERTAŞLAR transactions intact
  const otherTxs = fbData.transactions.filter(t => t.supplier !== 'ERTAŞLAR');
  fbData.transactions = [...otherTxs, ...newTransactions];

  // Merge priceLists
  if (!fbData.priceLists) fbData.priceLists = {};
  Object.assign(fbData.priceLists, newPriceLists);

  await setDoc(docRef, fbData);
  console.log('✅ Firebase successfully updated with 100% EXACT numbers from Excel!');
  process.exit(0);
}
