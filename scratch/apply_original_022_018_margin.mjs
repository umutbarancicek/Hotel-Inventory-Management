...
async function applyOriginal022Margin() {
  console.log('=== APPLYING EXACT ORIGINAL 0.22 / 0.18 MARGIN FORMULA TO TRANSACTIONS ===');
  ...
  transactions.forEach(tx => {
    ...
    if (tutedVal > 0) {
      ...
      tx.supplyPrice = calcSupply;
      updatedCount++;
    } else {
      const calcSupply = Math.round(tx.buyPrice * marginRate * 100) / 100;
      tx.supplyPrice = calcSupply;
    }
  });

  console.log(`Updated ${updatedCount} transactions with exact original 0.22 / 0.18 margins.`);

  const havucTxs = transactions.filter(t => t.date === '2026-08-04' && cleanStr(t.product).includes('HAVUC'));
  console.log('\nSample 04.08.2026 Havuç Beypazarı transactions after original 0.22 margin rule:');
  havucTxs.forEach(t => {
    console.log(`Supplier: ${t.supplier} | Hotel: ${t.hotel} | Product: ${t.product} | Buy: ₺${t.buyPrice} | Supply (TÜTED x 0.22): ₺${t.supplyPrice}`);
  });
  ...
  console.log(`\n=== ORIGINAL MARGIN RULE SYSTEM TOTALS ===`);
  ...
  console.log('✅ Firebase successfully updated with original 0.22 / 0.18 margins!');
}

applyOriginal022Margin().catch(console.error);
