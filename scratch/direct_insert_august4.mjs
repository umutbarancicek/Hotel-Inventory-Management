...
async function main() {
  console.log('Fetching price list for 2026-08-04...');
  const priceListSnap = await getDoc(doc(db, 'priceLists', '2026-08-04'));
  const targetTutedPrices = priceListSnap.exists() ? priceListSnap.data().prices || {} : {};
  ...
  console.log(`Current transaction count in DB: ${txs.length}`);

  // Filter out any existing manual ones on target date (just in case)
  // Let's uniquely identify them using IDs starting with 17869999 or simply append them
  const cleanedTxs = txs.filter(t => {
    // Only remove if they are from targetDate (2026-08-04), supplier MSD07 TAR ÜR, and were added manually (we will give them IDs starting with 1786500)
    return !(t.date === date && t.supplier === supplier && t.id >= 1786500000000 && t.id <= 1786500002000);
  });

  const newTransactions = [];
  let idBase = 1786500000000; // Let's use a clear fixed range so they can easily be identified or deleted
  ...
  console.log(`Saving ${updatedTxs.length} transactions...`);

  await updateDoc(appDataRef, { transactions: updatedTxs });
  console.log('✅ Saved. Let\'s verify...');

  const verifySnap = await getDoc(appDataRef);
  const verifyTxs = verifySnap.data().transactions || [];
  console.log(`Verified count in DB: ${verifyTxs.length}`);
  
  const checkInserted = verifyTxs.filter(t => t.date === date && t.supplier === supplier && t.id >= 1786500000000);
  console.log(`Inserted count verified: ${checkInserted.length}`);
}
