...
async function restoreCleanExactDates() {
  console.log('=== RESTORING 100% CLEAN AND ACCURATE DATES FROM ORIGINAL SOURCE FILES ===');
  ...
  let p1Count = 0;
  otelRows.forEach(r => {
    const d = excelSerialToIsoDate(r[1]);
  ...
  let p2Count = 0;
  ertRows.forEach(r => {
    const d = excelSerialToIsoDate(r[0]);
  ...
  console.log('Sample Clean Dates (ISO):', uniqueIsoDates);
  ...
  console.log('✅ Firebase DB successfully restored with 100% clean, exact, unswapped dates!');
}

restoreCleanExactDates().catch(console.error);
