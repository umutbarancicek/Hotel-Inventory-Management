...
// Complete Alias Map matching Ertaşlar Excel names -> TÜTED PDF & Web names
...
// Parse PDF file into product price map (divided by 10 for TL)
  ...
  for (const line of lines) {
    ...
    if (parts.length >= 3) {
      ...
      if (!isNaN(rawVal) && rawVal > 0) {
        // PDF prices are multiplied by 10 (e.g. 210,00 -> 21.00 TL)
...
// Fetch TÜTED Web Index for remaining dates
...
async function main() {
  console.log('Reading PDF files from Desktop/tüted...');
  const pdfPriceMaps = {};
  ...
  for (const tx of fbData.transactions) {
    ...
    if (pdfName && pdfPriceMaps[pdfName]) {
      ...
      if (tutedPriceTL !== null) {
        pdfMatchCount++;
        matchSource = `PDF (${pdfName})`;
      }
    }
    
    // 2. Web TÜTED match (July dates)
    if (tutedPriceTL === null) {
      ...
      if (tutedPriceTL !== null) {
        webMatchCount++;
        matchSource = 'Web TÜTED';
      }
    }
    
    if (tutedPriceTL !== null) {
      tx.supplyPrice = Math.round(tutedPriceTL * MARGIN * 100) / 100;
    } else {
      fallbackCount++;
      // If no TÜTED price found at all, estimate buyPrice * 1.82
      tx.supplyPrice = Math.round(tx.buyPrice * MARGIN * 100) / 100;
      matchSource = 'Fallback';
    }

    if (sampleCalculations.length < 8) {
      sampleCalculations.push({
        date: tx.date,
        hotel: tx.hotel,
        product: tx.product,
        buyPrice: tx.buyPrice,
        tutedPriceTL: tutedPriceTL ? tutedPriceTL.toFixed(2) : '-',
  ...
  console.log(`\n=== FINAL RECALCULATION RESULTS ===`);
  console.log(`Total ERTAŞLAR transactions: ${pdfMatchCount + webMatchCount + fallbackCount}`);
  console.log(`Matched via PDF files: ${pdfMatchCount}`);
  console.log(`Matched via Web TÜTED: ${webMatchCount}`);
  console.log(`Fallback (buyPrice * 1.82): ${fallbackCount}`);

  console.log('\nSample Updated Transactions:');
  sampleCalculations.forEach(s => {
    console.log(`  ${s.date} | ${s.hotel} | ${s.product} | Alış: ${s.buyPrice} TL | TÜTED: ${s.tutedPriceTL} TL | Tedarik (%182): ${s.supplyPrice} TL | Source: ${s.source}`);
  });

  console.log('\nSaving updated data to Firebase...');
  await setDoc(docRef, fbData);
  console.log('✅ Firebase successfully updated with accurate PDF TÜTED prices!');
  process.exit(0);
}
