...
// Convert any stored price to REAL TL PRICE per KG/unit (e.g. 45.00 TL)
function toRealTlPrice(rawPrice) {
  let val = parsePrice(rawPrice);
  if (val <= 0) return 0;

  // Real vegetable/fruit hal price per kg in Antalya Hal is strictly between 0.50 TL and 250.00 TL
  while (val > 250.0) {
    val = val / 10;
  }
...
async function fixFirebaseAll() {
  console.log('=== FIXING FIREBASE PRICELISTS & TRANSACTIONS WITH FIXED MAIN.JS LOGIC ===');
...
fixFirebaseAll().catch(console.error);
